...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if ([email, username, password].some((field) => field?.trim() === "")) {
        return res
            .status(400)
            .json(new ApiError(500, "All fields are required"))
    }
    ...
    if (existedUserByUsername) {
        return res
            .status(409)
            .json(new ApiError(409, "Username Already Taken"))
    }
...
export { registerUser, loginUser, logoutUser, refreshAccessToken, getCurrentUser, getUserChannelProfile };
